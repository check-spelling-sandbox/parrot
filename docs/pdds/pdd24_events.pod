...
A minimum threshold of priority, below which it will ignore the event even if
it is the right type. The default threshold is 0, accepting all event
priorities.
...
Set the minimum threshold of priority.
