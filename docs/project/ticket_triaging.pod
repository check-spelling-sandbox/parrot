...
Where I<New> refers to a preexisting ticket sitting in the Parrot queue with a
status of C<new>.
...
=item Is the defect reproducible in the current development version?

Reproducable defects have a normal priority.  Unreproducible tickets have a
lower priority.
