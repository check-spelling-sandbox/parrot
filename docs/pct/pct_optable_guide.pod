...
This can be accomplished using the C<pirop> trait, discussed below.
...
Operators have a certain associativity. For instance, the C<+> operator
...
This associativity is expressed using the C<assoc> trait, like so:
...
There is a third associativity in Perl 6. Generally a list (using the comma
